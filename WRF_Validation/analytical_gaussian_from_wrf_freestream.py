"""
Run the Bastankhah-Porte-Agel Gaussian wake model for every WRF wrfout case.

The driver reads WRF hub-height inflow, runs PyWake on the WRF mass grid, and
saves one PNG, NPZ, and turbine CSV per case plus a summary CSV.
"""
...
from wrf_wake_utilities import (
    ...
    curve_value,
    estimate_freestream_from_wrf,
    extract_hub_wind_components,
    find_wrfout_files,
...
DEFAULT_OUTPUT_DIR = SCRIPT_DIR / "outputs" / "gaussian"
...
def clean_gaussian_speed(speed_2d: np.ndarray, freestream_speed: float) -> np.ndarray:
...
def simulate_gaussian_on_wrf_domain(
    ...
    return simulate_pywake_on_wrf_domain(
        ...
        model_factory=lambda site, wt: Bastankhah_PorteAgel_2014(site, wt, k=float(wake_expansion_k)),
        turbulence_intensity=turbulence_intensity,
        model_key="gaussian",
    )


def plot_gaussian_png(
    output_file: Path,
    ...
    """Save one Gaussian wake heatmap as a PNG file."""
...
def save_case_outputs(
    ...
    """Save one Gaussian case as PNG, NPZ, and turbine CSV."""
    ...
    return save_pywake_case_outputs(
        ...
        model_key="gaussian",
        model_title="Gaussian",
        extra_npz={"gaussian_k": float(plot_settings.wake_expansion_k)},
        extra_summary={"gaussian_k": float(plot_settings.wake_expansion_k)},
    )


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for WRF-driven Gaussian batch processing."""

    parser = argparse.ArgumentParser(
        description="Run PyWake Gaussian for every processed WRF case."
    )
    add_common_wrf_model_args(parser, DEFAULT_OUTPUT_DIR, "PNG/NPZ/CSV")
    parser.add_argument("--wake-expansion-k", type=float, default=0.0324555, help="Gaussian wake expansion coefficient k.")
    parser.add_argument("--turbulence-intensity", type=float, default=0.15, help="Ambient turbulence intensity passed to PyWake.")
    parser.add_argument("--plot-normalized", action="store_true", help="Plot normalized wind speed instead of m/s.")
    return parser.parse_args()


def main() -> None:
    """Run the full Gaussian batch workflow."""
    ...
    print(f"Saving Gaussian PNG/NPZ/CSV outputs to {output_dir}")
    ...
    for file_index, wrfout in enumerate(wrfout_files, start=1):
        ...
        result = simulate_gaussian_on_wrf_domain(
            turbines=turbines,
            metadata=metadata,
            ...
            curve=curve,
            pywake_turbines=pywake_turbines,
            wake_expansion_k=args.wake_expansion_k,
        ...
        print(
            ...
            f"Gaussian min/mean/max={row['gaussian_u_min_mps']:.3f}/"
            f"{row['gaussian_u_mean_mps']:.3f}/{row['gaussian_u_max_mps']:.3f} m/s; "
            f"power={row['total_power_mw']:.3f} MW"
        )

    summary_file = output_dir / "gaussian_from_wrf_summary.csv"
